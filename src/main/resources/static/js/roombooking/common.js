/* ============================================================
   roombooking/common.js — 共用常數與工具
   內容:STATUS 對照表、日期工具、api()、toast、log、
        訂單付款倒數、兩張訂單表共用的明細/統計函式
   ------------------------------------------------------------
   拆檔的接法:每個功能檔把自己的 computed/methods 掛在 RB.xxx 上,
   main.js 最後用展開運算子 {...RB.common.methods, ...RB.booking.methods, ...}
   合併進同一個 Vue app——效果跟原本全寫在一支檔案完全一樣。
   載入順序(roombooking.html 檔尾):common.js 一定要排第一
   (它建立 RB 命名空間),main.js 一定要排最後(它做合併)。
   ============================================================ */
window.RB = window.RB || {};
