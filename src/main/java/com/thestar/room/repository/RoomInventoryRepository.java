...
public interface RoomInventoryRepository extends JpaRepository<RoomInventoryVO, RoomInventoryId> {
	...
	void initInventory(@Param("date") LocalDate date, @Param("roomTypeId") Integer roomTypeId);
	...
	int bookRooms(@Param("date") LocalDate date, @Param("roomTypeId") Integer roomTypeId, @Param("qty") int qty);
	...
	int releaseRoom(@Param("date") LocalDate date, @Param("roomTypeId") Integer roomTypeId, @Param("qty") int qty);

	// redis用來查詢目前庫存房間數有多少
	@Query(value = "SELECT TOTAL_AMOUNT-BOOKED_AMOUNT FROM ROOM_INVENTORY "
			+ "WHERE INVENTORY_DATE = :date AND ROOM_TYPE_ID = :roomTypeId", nativeQuery = true)
	Integer checkInventory(@Param("roomTypeId") Integer roomTypeId, @Param("date") LocalDate date);

	List<RoomInventoryVO> findByIdInventoryDateGreaterThanEqualAndIdInventoryDateLessThan(LocalDate checkInDate,
			LocalDate checkOutDate);

	// 使用 JPQL 明確指定路徑：透過 r.id (EmbeddedId) 存取裡面的 roomTypeId 和 inventoryDate
	// 刪除房型前檢查未來庫存用
	@Query("SELECT CASE WHEN COUNT(r) > 0 THEN TRUE ELSE FALSE END FROM RoomInventoryVO r "
			+ "WHERE r.id.roomTypeId = :roomTypeId AND r.id.inventoryDate >= :date")
	boolean existsByRoomTypeIdAndDate(@Param("roomTypeId") Integer roomTypeId, @Param("date") LocalDate date);
